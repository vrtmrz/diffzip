import { AbstractInputSuggest, Modal, prepareFuzzySearch, Setting, type App, type SearchResult, type TextComponent } from "obsidian";
...
export class RestoreDialog extends Modal {
	...
	constructor(app: App, public plugin: DiffZipBackupPlugin) {
		super(app);
	}
	...
	async onOpen() {
		...
		this.currentFiles = [];
		// Debug
		// this.currentFiles = [".obsidian/plugins/diffzip/main.js", "Welcome.md"]
		...
		function getFiles() {
			...
			const dirs = [...new Set(filesAll.map(e => (e.split("/").slice(0, -1).join("/") + "/").slice(0, -1)))].map(e => e + "/*").map(e => e == "/*" ? "*" : e);
		...
		new Setting(footerEl)
			...
			.addButton(b => {
				b.setButtonText("Restore").onClick(async () => {
					// this.plugin.restoreFiles(this.currentFiles, this.selectedTimestamps);
					this.close();
					...
					const applyFiles = Object.fromEntries(
						Object.entries(selected)
							.map(([file, timestamp]) => file.endsWith("*") ?
								allFiles.filter(e => e.startsWith(file.slice(0, -1))).map(
									file => ([file, timestamp] as const))
								: [[file, timestamp]] as const).flat());
...
class PopTextSuggest extends AbstractInputSuggest<TextSearchResult> {
	...
	// items = ["abc", "bd", "caa", "ab", "bc", ...[...new Array(200)].map((e, i) => `${i}`)] as string[];
	...
	protected getSuggestions(query: string): TextSearchResult[] | Promise<TextSearchResult[]> {
		...
		const p = this.items.map(e => ({ result: q(e), source: e })).filter(e => e.result !== null) as { result: SearchResult, source: string }[];
	...
	renderSuggestion(value: TextSearchResult, el: HTMLElement): void {
		...
		const highlighted = source.map(e => false);
