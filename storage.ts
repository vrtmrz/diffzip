...
export class NormalVault extends StorageAccessor {
	...
	async checkType(path: string): Promise<FileType> {
		// const existence = this.app.vault.adapter.exists(path);
...
export function getStorageInstance(type: StorageAccessorType, plugin: DiffZipBackupPlugin, basePath?: string, isLocal?: boolean): StorageAccessor {
	if (type == "external") {
