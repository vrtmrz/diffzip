...
export function confirmWithMessage(
    ...
    return new Promise((res) => {
        const dialog = new MessageBox(plugin, title, contentMd, buttons, defaultAction, false, (result) => res(result));
        dialog.open();
    });
}
...
export const askSelectString = (app: App, message: string, items: string[]): Promise<string> => {
    return new Promise((res) => {
        const popOver = new PopOverSelectString(app, message, "", () => items, res);
        popOver.open();
    });
};
