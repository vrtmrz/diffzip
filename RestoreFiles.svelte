<script lang="ts">
    ...
    let { plugin, toc = $bindable(), fileList, selectedTimestamp }: Props = $props();
    const files = $derived($fileList.sort((a, b) => a.localeCompare(b, undefined, { numeric: true })));
    ...
    function expandFolder(name: string, preventRender = false) {
        ...
        const newFiles = [...new Set([...$fileList, ...files])].filter((e) => e !== name);
...
<div class="diff-zip-files">
    {#if files}
        {#each files as file (file)}
            <RestoreFileInfo {commands} {plugin} {toc} filename={file} selected={$selectedTimestamp?.[file] ?? 0} />
        {/each}
    {/if}
</div>
