<script lang="ts">
    ...
    import { LATEST, type ListOperations } from "./RestoreView.ts";
    ...
    let { toc = $bindable(), filename = $bindable(), commands = $bindable(), selected = $bindable() }: Props = $props();

    const isFolder = $derived(filename.endsWith("*"));
    const relatedFiles = $derived(
        isFolder ? Object.keys(toc).filter((f) => f.startsWith(filename.slice(0, -1))) : [filename]
    );
    const relatedFilesInfo = $derived(relatedFiles.map((f) => toc[f]));
    const timeStamps = $derived(
        [...new Set(relatedFilesInfo.map((f) => f.history.map((e) => new Date(e.modified).getTime())).flat())].sort(
            (a, b) => b - a
        )
    );
...
<div class="diffzip-list-row">
    ...
    <span class="diffzip-list-timestamp">
        ...
        {:else}
            <select
                class="dropdown"
                onchange={(e) =>
                    commands.fileSelected(filename, Number.parseInt((e.target as HTMLSelectElement)?.value))}
                value={selected}
            >
    ...
    <span class="diffzip-list-actions">
        {#if isFolder}
            <button title="Expand Folder" onclick={() => commands.expandFolder(filename)}> 📂 </button>
        {/if}
